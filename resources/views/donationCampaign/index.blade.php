...
@section('content')
    ...
    @if (session('success campaign register'))
        <script>
            popUpMessageDelay('{{ session('success campaign register') }}')
        </script>
    @endif

    @if (session('error server conection'))
        <script>
            popUpMessageDelay('{{ session('error server conection') }}')
        </script>
    @endif

    @if (session('donation completed'))
        <script>
            popUpMessageDelay('{{ session('donation completed') }}')
        </script>
    @endif
    ...
    <div class="text-pages font-bold">
        ...
        <form class="mb-5 grid grid-cols-20-80 grid-rows-1 justify-center" action="{{ route('donation-campaign.filter') }}"
        ...
        @role('admin')
            <a class="p-5 text-2xl border-2 border-solid border-white w-40 mt-1 hover:bg-sky-700"
                href="{{ route('donation-campaign.create') }}">
                <button class="mb-10">
                    Cargar campaña
                </button>
            </a>
                            ...
                            <td class="w-36 text-center">
                                ...
                                @if ($campaign->state->value == App\Enums\DonationCampaignStatesEnum::ACTIVE->value)
                                    @role('admin')
                                        ...
                                        <form id="finish-form{{ $campaign->id }}"
                                            action="{{ route('donation-campaign.finish', $campaign) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <button class="mb-2" type="submit"
                                                onclick="event.preventDefault(); confirmationPopUp('¿Está seguro de finalizar la campaña?', 'finish-form{{ $campaign->id }}')">
                                                Finalizar campaña
                                            </button>
                                        </form>

                                        <a class="mb-2" href="{{ route('donation-campaign.records', $campaign) }}">
                                            <button type="submit">
                                    ...
                                    @else
                                        <button type="submit">
                                            <a
                                                href="{{ route('donation-campaign.donate', ['campaign_id' => $campaign->id]) }}">
                                                Donar </a>
                                        </button>
                                    @endrole
                                @else
